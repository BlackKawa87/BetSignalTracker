...
import {
  ...
  AlertTriangle, Database, Eye, EyeOff, FlaskConical, Wrench,
} from 'lucide-react'
...
const LABEL = 'text-xs mb-1 block text-[color:var(--color-text-muted)] uppercase tracking-wider font-medium'
const SECTION_TITLE = 'text-sm font-semibold text-[color:var(--color-text-primary)] flex items-center gap-2 mb-4'
const SUBTLE_BTN =
  'flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-lg border border-[color:var(--color-border)] text-[color:var(--color-text-secondary)] hover:text-[color:var(--color-text-primary)] hover:border-[color:var(--color-border-strong)] transition-colors disabled:opacity-40'

// ── Section: Advanced Mode toggle ─────────────────────────────

function AdvancedModeSection() {
  const [enabled, setEnabled] = useState(() => localStorage.getItem('bst_advanced') === 'true')

  const toggle = () => {
    const next = !enabled
    setEnabled(next)
    localStorage.setItem('bst_advanced', String(next))
    window.location.reload()
  }

  return (
    <Card className="p-5">
      <h2 className={SECTION_TITLE}><Wrench size={14} className="text-brand" /> Modo Avançado</h2>
      <div className="flex items-start gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-[color:var(--color-text-muted)] mt-1 leading-relaxed">
            Exibe Laboratório de Testes, Estatísticas detalhadas e logs técnicos no menu lateral.
          </p>
        </div>
        <button
          onClick={toggle}
          ...
          style={{
            background: enabled ? '#00d084' : 'var(--color-input-border)',
          }}
        >
          <span
...
function BankrollSection({
  ...
  return (
    <Card className="p-5 space-y-4">
      <h2 className={SECTION_TITLE}><DollarSign size={14} className="text-brand" /> Gestão de Banca</h2>
      ...
      {initialBankroll > 0 && currentBankroll > 0 && (
        <div className="flex items-center gap-3 p-3 rounded-lg bg-[color:var(--color-nav-hover-bg)]">
          <div>
            <p className="text-[10px] text-[color:var(--color-text-muted)] uppercase tracking-wider">P&L Total</p>
          ...
          <div>
            <p className="text-[10px] text-[color:var(--color-text-muted)] uppercase tracking-wider">ROI</p>
      ...
      <label className="block">
        ...
        {stakePreview > 0 && (
          <p className="text-xs text-[color:var(--color-text-muted)] mt-1">
            → Stake por aposta: <span className="text-brand font-mono">{formatCurrency(stakePreview)}</span>
          </p>
        )}
      </label>
    </Card>
  ...
  return (
    <Card className="p-5 space-y-4">
      ...
      <label className="block">
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[color:var(--color-text-muted)] hover:text-[color:var(--color-text-secondary)]"
          >
            {showToken ? <EyeOff size={14} /> : <Eye size={14} />}
          </button>
        </div>
        <p className="text-[10px] text-[color:var(--color-text-muted)] mt-1">
          O token também deve ser configurado como <span className="text-[color:var(--color-text-secondary)] font-mono">TELEGRAM_BOT_TOKEN</span> nas variáveis de ambiente da Vercel.
        </p>
      </label>
      ...
      <label className="block">
        ...
        {webhookUrl && (
          <p className="text-[10px] text-[color:var(--color-text-muted)] mt-1">
            Webhook URL: <span className="text-[color:var(--color-text-secondary)] font-mono break-all">{webhookUrl}</span>
          </p>
        )}
      </label>

      <div className="flex items-center gap-2 flex-wrap">
        <button onClick={testTelegram} disabled={testing} className={SUBTLE_BTN}>
          {testing ? <RefreshCw size={12} className="animate-spin" /> : <Bot size={12} />}
          {testing ? 'Testando...' : 'Testar conexão'}
        </button>

        <button onClick={configureWebhook} disabled={settingWh || !deployUrl} className={SUBTLE_BTN}>
      ...
      {whResult && (
        <div className={`p-3 rounded-lg text-xs font-mono break-all ${whResult.startsWith('Webhook') ? 'bg-accent-green/5 border border-accent-green/20 text-accent-green' : 'bg-accent-red/5 border border-accent-red/20 text-accent-red'}`}>
          {whResult}
        </div>
      )}
    </Card>
  ...
  return (
    <Card className="p-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {[
          { label: 'Sinais — CSV',  sub: `${signals.length} registros`,           action: () => exportSignalsCsv(signals) },
          { label: 'Sinais — JSON', sub: `${signals.length} registros`,           action: () => exportSignalsJson(signals) },
          { label: 'Banca — CSV',   sub: `${bankrollHistory.length} entradas`,    action: () => exportBankrollCsv(bankrollHistory) },
          { label: 'Analytics — CSV', sub: 'Resumo por mercado',                  action: () => exportAnalyticsCsv(signals) },
          {
          ...
          <button
            key={item.label}
            onClick={item.action}
            className="flex items-center gap-3 p-3 rounded-lg text-left transition-colors bg-[color:var(--color-nav-hover-bg)] border border-[color:var(--color-border)] hover:border-[color:var(--color-border-strong)]"
          >
            <Download size={14} className="text-brand flex-shrink-0" />
            <div>
              <p className="text-sm text-[color:var(--color-text-primary)] font-medium">{item.label}</p>
              <p className="text-[10px] text-[color:var(--color-text-muted)] mt-0.5">{item.sub}</p>
            </div>
          </button>
        ))}
      </div>
...
function DemoSection() {
  ...
  const seed = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        await refreshAll()
      } else { setResult(`Erro: ${data.error ?? 'desconhecido'}`) }
  ...
  return (
    <Card className="p-5">
      <h2 className={SECTION_TITLE}><FlaskConical size={14} className="text-cyan-500" /> Modo Demo</h2>
      <p className="text-xs text-[color:var(--color-text-muted)] mb-4 leading-relaxed">
        Popula o sistema com 30 sinais fictícios para testar analytics e o dashboard sem dados reais.
        Sinais demo são marcados com <span className="text-[color:var(--color-text-secondary)] font-mono">[DEMO]</span> nas notas.
      </p>
      <div className="flex items-center gap-2">
        <button
          onClick={seed}
          disabled={seeding}
          className="flex items-center gap-1.5 px-4 py-2 text-sm bg-cyan-500/10 border border-cyan-500/30 text-cyan-600 dark:text-cyan-400 hover:bg-cyan-500/20 rounded-lg transition-colors disabled:opacity-40"
        >
          {seeding ? <RefreshCw size={13} className="animate-spin" /> : <Database size={13} />}
          {seeding ? 'Inserindo...' : 'Inserir dados demo'}
        </button>
        <button onClick={clearDemo} disabled={clearing} className={SUBTLE_BTN}>
          {clearing ? <RefreshCw size={13} className="animate-spin" /> : <Trash2 size={13} />}
          {clearing ? 'Limpando...' : 'Limpar dados demo'}
        </button>
      </div>
      {result && (
        <p className="text-xs mt-3 font-mono text-[color:var(--color-text-secondary)]">{result}</p>
      )}
    </Card>
  )
}
...
function DangerZone() {
  ...
  const clearAll = async () => {
    ...
    setDeleting(false); setConfirm1(false); setConfirm2(false)
  }

  const label = !confirm1 ? 'Apagar todos os dados'
    : !confirm2 ? 'Tem certeza? Clique de novo'
    : 'Clique para confirmar — IRREVERSÍVEL'

  return (
    <Card className="p-5 border-red-200 dark:border-red-400/20">
      ...
      <p className="text-xs text-[color:var(--color-text-muted)] mb-4">
        Remove <strong className="text-[color:var(--color-text-secondary)]">todos os sinais e histórico de banca</strong>.
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg transition-all ${
          confirm2 ? 'bg-red-500/20 border border-red-500/50 text-red-600 dark:text-red-400'
          : confirm1 ? 'bg-orange-500/10 border border-orange-500/30 text-orange-600 dark:text-orange-400'
          : 'border border-[color:var(--color-border)] text-[color:var(--color-text-muted)] hover:text-accent-red hover:border-red-400/30'
        }`}
      >
...
export function SettingsPage() {
  ...
  const [form, setForm] = useState<Record<string, string>>({
    initial_bankroll: '', current_bankroll: '', stake_percentage: '',
    preferred_bookmaker: '', main_strategy: '',
    telegram_bot_token: '', deploy_url: '',
  })
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <PageHeader
        title="Configurações"
        subtitle="Gerencie sua banca, integrações e preferências da plataforma."
      />
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="flex items-center gap-2 px-6 py-2.5 bg-brand text-white font-semibold rounded-lg hover:opacity-90 transition-opacity disabled:opacity-40"
      >
